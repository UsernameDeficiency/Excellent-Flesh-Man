...
;Används endast för debugging
(define (what?)
  (printf "Upper left left collision: ~a"
          (collision? (- (send *player* get-x-pos) 1) (send *player* get-y-pos)))
  (newline)
  (printf "Upper left up collision: ~a"
          (collision? (send *player* get-x-pos) (- (send *player* get-y-pos) 1)))
  (newline)
  (printf "Upper right right collision: ~a"
          (collision? (+ (send *player* get-x-pos) 20) (send *player* get-y-pos)))
  (newline)
  (printf "Upper right up collision: ~a"
          (collision? (+ (send *player* get-x-pos) 19) (- (send *player* get-y-pos) 1)))
  (newline)
  (printf "Lower left left collision: ~a"
          (collision? (- (send *player* get-x-pos) 1) (+ (send *player* get-y-pos) 19)))
  (newline)
  (printf "Lower left down collision: ~a"
          (collision? (send *player* get-x-pos) (+ (send *player* get-y-pos) 20)))
  (newline)
  (printf "Lower right right collision: ~a"
          (collision? (+ (send *player* get-x-pos) 20) (+ (send *player* get-y-pos) 19)))
  (newline)
  (printf "Lower right down collision: ~a"
          (collision? (+ (send *player* get-x-pos) 19) (+ (send *player* get-y-pos) 20)))
  (newline)
  (printf "Four Corners collission: ~n~a ~a~n~a ~a~n" 
          (collision? (send *player* get-x-pos) (send *player* get-y-pos))
          (collision? (+ (send *player* get-x-pos) 19) (send *player* get-y-pos))
          (collision? (send *player* get-x-pos) (+ (send *player* get-y-pos) 19))
          (collision? (+ (send *player* get-x-pos) 19) (+ (send *player* get-y-pos) 19)))
  (printf "Allowed jumps: ~a~n" (send *player* get-allowed-jumps))
  (printf "X-Speed: ~a~n" (send *player* get-x-speed))
  (printf "Y-Speed: ~a~n" (send *player* get-y-speed))
  (printf "X-Pos - X-New: ~a~n" (- (send *player* get-x-pos) (+ (send *player* get-x-speed) (send *player* get-x-pos))))
  (printf "Y-Pos - Y-New: ~a~n" (- (send *player* get-y-pos) (+ (send *player* get-x-speed) (send *player* get-y-pos)))))
